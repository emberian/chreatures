...
impl RouteGeometry {
    ...
    pub fn new(fixture: &str) -> Result<Self, String> {
        #[derive(Deserialize)]
        struct Config { ecology: EcologyConfig }
    ...
    pub fn refresh_due(&self, tick: u32, topology: u32) -> Result<bool, String> {
        self.state.refresh_due(&self.plan, tick.into(), topology.into()).map_err(|e| e.to_string())
    }
    pub fn measure(&mut self, tick: u32, topology: u32, plan_sha256: &str,
        distances_m: &[f64], inside: &[u8]) -> Result<Vec<f64>, String> {
        if inside.iter().any(|x| *x > 1) { return Err("endpoint containment must be binary".into()); }
        let inside = inside.iter().map(|x| *x != 0).collect::<Vec<_>>();
        self.state.refresh(&self.plan, tick.into(), topology.into(), plan_sha256,
            distances_m, &inside).map_err(|e| e.to_string())?;
        self.openness(tick, topology)
    }
    pub fn openness(&self, tick: u32, topology: u32) -> Result<Vec<f64>, String> {
        self.state.openness(&self.plan, tick.into(), topology.into())
            .map(|x| x.to_vec()).map_err(|e| e.to_string())
    }
    pub fn snapshot(&self) -> String {
        serde_json::to_string(&self.state).unwrap()
    }
    pub fn validate_snapshot(&self, snapshot: &str) -> Result<(), String> {
        let state: RouteGeometryState = serde_json::from_str(snapshot).map_err(|e| e.to_string())?;
