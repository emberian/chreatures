...
class Chemistry:
    ...
    def __init__(self, value: Mapping[str, Any]):
        ...
        for names in (self.elements, self.pools, self.reactions):
            if not names or len(set(names)) != len(names) or any(
                not isinstance(name, str) or not name or len(name) > 96 for name in names
            ):
        ...
        for row, reaction in enumerate(self._value["reactions"]):
            for field, sign in (("consume", -1), ("produce", 1)):
                for name, amount in reaction[field].items():
                    if name not in self.pools or not np.isfinite(amount) or float(amount) < 0:
        ...
        self._arrays = (
            ...
            np.asarray([p["chemical_energy"] for p in self._value["pools"]], np.float64),
            *[
                np.asarray([item[name] for item in self._value["reactions"]], np.float64)
    ...
    def enzymes(self, rows: Sequence[Mapping[str, float]]) -> np.ndarray:
        ...
        values = np.asarray([[row.get(name, 0.0) for name in self.reactions] for row in rows], np.float64)
...
class MetabolicWeb:
    ...
    def __init__(
        self, chemistry: Chemistry, enzymes: Sequence[Mapping[str, float]],
        pools: Sequence[Mapping[str, float]], atp: Sequence[float],
        ...
        self._native = load_world_kernels().MetabolicCohort(
            *chemistry._arrays, chemistry.enzymes(enzymes),
            ...
            chemistry.resources(bulk or {}), float(bulk_atp),
        )
    ...
    def totals(self) -> dict[str, Any]:
        ...
        energy = resources @ self.chemistry._arrays[2] + self.atp.sum() + self._native.bulk_atp
        return {"elements": dict(zip(self.chemistry.elements, elements.tolist())), "stored_energy": float(energy)}

    def step(self, dt: float, photons: Any, work: Any) -> dict[str, np.ndarray]:
        return {
            name: np.asarray(value)
            for name, value in self._native.step(
                float(dt), np.ascontiguousarray(photons, dtype=np.float64),
                np.ascontiguousarray(work, dtype=np.float64),
            ).items()
        }

    def transfer(
        self, donor: int | None, receiver: int | None,
        resources: Mapping[str, float], *, atp: float = 0.0,
    ) -> None:
        self._native.transfer(donor, receiver, self.chemistry.resources(resources), float(atp))
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            "format": SNAPSHOT_FORMAT, "chemistry": self.chemistry.to_value(),
            "chemistry_sha256": self.chemistry.sha256, "program_sha256": self.program_sha256,
            "compartments": self.count, "native_sha256": hashlib.sha256(data).hexdigest(),
    ...
    def restore(cls, value: Mapping[str, Any]) -> "MetabolicWeb":
        ...
        instance = cls(chemistry, [{}] * count, [{}] * count, [0.] * count, [0.] * count)
