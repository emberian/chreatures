...
def create_app(
    ...
    checkpoint = checkpoint or ROOT / (
        "runs/hollow-garden.json" if dimension == 3 else "runs/residents.json"
    )
    ...
    async def lifespan(app):
        try:
            if dimension == 3:
                ...
                authority["habitat"] = (
                    Habitat.load(checkpoint) if checkpoint.exists() else Habitat(seed)
                )
    ...
    def index():
        return FileResponse(
            ROOT / ("web/garden.html" if dimension == 3 else "web/index.html")
        )
...
def main(default_dimension=2):
    ...
    parser.add_argument(
        "--dimension", type=int, choices=(2, 3), default=default_dimension
    )
