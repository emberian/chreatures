...
def complete_unit_metrics(predicted: np.ndarray, observed: np.ndarray, indices: np.ndarray,
    ...
    for local, row in enumerate(indices):
        key = tuple(str(data[name][row]) for name in
...
def fit(data_path: Path, schema_path: Path, feature_contract_path: Path, output: Path) -> dict:
    ...
    terms = " + ".join(f"s({name}, k={int(schema.get('basis_size', 9))})" for name in names)
        ...
        try:
            ...
            for split_name, mask in (("validation", validation),
                ...
                metrics[split_name] = {"rows": int(mask.sum()),
                    ...
                    **complete_unit_metrics(predicted, observed, np.flatnonzero(mask), data)}
            residual = transformed[test] - np.asarray(model.predict([{name: float(z[row, col]) for col, name in enumerate(names)}
                for row in np.flatnonzero(test)]), dtype=float)
