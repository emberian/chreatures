...
RICH_DEVELOPMENT_CHECKPOINT_FORMAT = "chreatures-rich-online-sensorimotor-development-v1"
...
RICH_MODEL_NAMES = (
    ...
    "history.weight_ih_l0", "history.weight_hh_l0", "history.bias_ih_l0",
    "history.bias_hh_l0", "policy_trunk.0.weight", "policy_trunk.0.bias",
    "signed_head.weight", "signed_head.bias", "active_head.weight", "active_head.bias",
    "positive_head.weight", "positive_head.bias",
)
RICH_ORDER = ("normalizer.mean", "normalizer.scale") + tuple(
    "model." + name for name in RICH_MODEL_NAMES
) + MANAGER_ORDER
RICH_SHAPES = {
    "normalizer.mean": (4453,), "normalizer.scale": (4453,),
    "model.visual.peripheral.first.weight": (16, 4, 3, 3), "model.visual.peripheral.first.bias": (16,),
    "model.visual.peripheral.second.weight": (24, 16, 3, 3), "model.visual.peripheral.second.bias": (24,),
    "model.visual.foveal.first.weight": (16, 4, 3, 3), "model.visual.foveal.first.bias": (16,),
    "model.visual.foveal.second.weight": (24, 16, 3, 3), "model.visual.foveal.second.bias": (24,),
    "model.visual.peripheral_projection.0.weight": (64, 768), "model.visual.peripheral_projection.0.bias": (64,),
    "model.visual.foveal_projection.0.weight": (64, 2304), "model.visual.foveal_projection.0.bias": (64,),
    ...
    "model.observation_projection.0.weight": (128, 265), "model.observation_projection.0.bias": (128,),
    "model.history.weight_ih_l0": (384, 128), "model.history.weight_hh_l0": (384, 128),
    ...
    "manager.query.2.weight": (64, 128), "manager.query.2.bias": (64,), "manager.query_gain": (1,),
}
...
def export_rich(checkpoint: dict[str, Any], checkpoint_path: Path, output: Path, laws_path: Path) -> None:
    ...
    if (
        tuple(arrays) != RICH_ORDER
        or any(arrays[name].shape != RICH_SHAPES[name] for name in RICH_ORDER)
        or any(not value.flags.c_contiguous or not np.isfinite(value).all() for value in arrays.values())
    ):
        raise ValueError("rich export tensor order, shape, or values differ")
    tensors = {
        name: {"dtype": "float32", "shape": list(value.shape),
               "sha256": hashlib.sha256(value.tobytes()).hexdigest()}
        for name, value in arrays.items()
    }
    law_bytes=laws_path.read_bytes(); law_bank=json.loads(law_bytes)
    if law_bank.get("schema")!="chreatures-gam-consequence-law-bank-v1": raise ValueError("GAM law schema differs")
    refinement={"law_bank":law_bank,"law_file_sha256":hashlib.sha256(law_bytes).hexdigest(),"law_content_sha256":hashlib.sha256(canonical(law_bank)).hexdigest(),"candidates":4,"tilt":0.5,"learning_rate":0.05,"error_decay":0.99,"innovation_limit":4.0}
    metadata = {
        "format": DEVELOPMENTAL_FORMAT,
        "version": 1,
        "execution": "developmental-resident-native-rich-v1",
        ...
        "observation_contract": {
            ...
            "source_sense_dim": 351, "physiology_dim": 6,
            "neural_readout_dim": 384, "previous_action_plus_oral_dim": 9,
        },
        "normalizer_sha256": normalizer_hash,
        "consequence_refinement": refinement,
        "training_identity": json.loads(canonical(identity)),
        "tensors": tensors,
        "pack_order": list(RICH_ORDER),
        "temporal_contract": {"observation_interval_seconds": 0.05,
    ...
    with temporary.open("wb") as stream:
        np.savez_compressed(stream, metadata=np.asarray(canonical(metadata).decode()), **arrays)
    temporary.replace(output)
    print(json.dumps({"path": str(output), "bytes": output.stat().st_size,
                      "file_sha256": sha256(output),
                      "artifact_sha256": metadata["artifact_sha256"],
                      "mode": "rich-achieved-goal"}, sort_keys=True))
...
def main() -> None:
    ...
    export_rich(checkpoint, checkpoint_path, args.output,args.consequence_laws.expanduser().resolve())
