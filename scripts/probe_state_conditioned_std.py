...
from chreatures.learning import (ACTIONS, MacroRollout, PredictivePPOConfig,
                                 PredictivePPOTrainer)  # noqa: E402


def make_rollout(trainer: PredictivePPOTrainer, rng: np.random.Generator,
                 steps: int = 5) -> MacroRollout:
    ...
    for step in range(steps):
        ...
        target = rng.normal(size=(count, cfg.projection_dim)).astype(np.float32) * .1
        reward = np.linspace(-.4, .7, count, dtype=np.float32) + step * .03
        rollout.append(features=previous["features"], physiology=previous["physiology"],
            context=previous["context"], latent=previous["latent"], action=previous["action"],
            log_prob=previous["log_prob"], value=previous["value"], reward=reward,
            done=np.zeros(count, np.float32), prediction_target=target)
    return rollout
...
def main() -> None:
    ...
    cfg = PredictivePPOConfig(feature_dim=12, physiology_dim=6, context_dim=5,
                              projection_dim=7, hidden_dim=16, epochs=2,
                              minibatch_size=8, seed=7781)
    legacy = PredictivePPOTrainer(["a", "b", "c"], cfg)
    legacy.update(make_rollout(legacy, rng), np.zeros(3, np.float32), .25)
    legacy.context[:] = rng.normal(size=legacy.context.shape).astype(np.float32)
    legacy.error_fast[:] = (.1, .2, .3)
    legacy.error_slow[:] = (.2, .3, .4)
    with tempfile.TemporaryDirectory() as directory:
        ...
        paired = {key: float(np.max(np.abs(old_action[key] - new_action[key])))
                  for key in ("latent", "action", "log_prob", "value", "prediction")}

        old_opt, new_opt = restored.optimizer.state_dict(), upgraded.optimizer.state_dict()
        legacy_ids = old_opt["param_groups"][0]["params"]
        optimizer_delta = max((float((old_opt["state"][old_id][key].cpu()
                                      - new_opt["state"][new_id][key].cpu()).abs().max())
                               for old_id, new_id in zip(
                                   legacy_ids, new_opt["param_groups"][0]["params"], strict=False)
                               for key in old_opt["state"].get(old_id, {})), default=0.0)
        new_ids = new_opt["param_groups"][0]["params"][len(legacy_ids):]
        new_optimizer_state_max = max((float(value.abs().max())
                                       for pid in new_ids
                                       for value in new_opt["state"][pid].values()), default=0.0)
        private_delta = max(float(np.max(np.abs(restored.context - upgraded.context))),
        ...
        probe_context = torch.as_tensor(np.stack((upgraded.context[0], upgraded.context[1])))
        with torch.no_grad():
        ...
        metrics = upgraded.update(make_rollout(upgraded, rng), np.zeros(3, np.float32), .25)
        with torch.no_grad():
            mean, _, hidden = upgraded.model(probe_features, probe_body, probe_context)
            std_after = upgraded.model.distribution(mean, hidden).scale.clone()
        head_change = float((upgraded.model.std_offset.weight.detach() - head_before).abs().max())
        ...
        replay_delta = max(float(np.max(np.abs(expected[key] - actual[key])))
                           for key in ("latent", "action", "log_prob", "value", "prediction"))
    report = {"actions": list(ACTIONS), "upgrade_profile": upgraded.config.std_profile,
              ...
              "private_state_max_abs_delta": private_delta, "upgrade_extra": extra,
              "ppo_update": metrics, "head_weight_max_change": head_change,
              "conditioned_std_between_states_max_abs_delta": conditioned_span,
              "std_after_vs_before_max_abs_delta": float((std_after - std_before).abs().max()),
