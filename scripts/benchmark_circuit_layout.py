...
from chreatures.circuit_layout import (  # noqa: E402
    CircuitRowLayout,
    ScheduledTritonCircuit,
    _scheduled_csr_rate_substep,
)
...
def locality(graph: MaleCNSGraph, order: np.ndarray, page: int) -> dict[str, float]:
    ...
    return {
        "adjacent_quartile_page_overlap_fraction": float(overlap.mean()),
        "sampled_rows": int(graph.n),
        "source_page_neurons": int(page),
    }
...
def main() -> None:
    ...
    p.add_argument(
        "--method",
        choices=("anatomy-source-page", "degree-descending"),
    ...
    if args.layout.exists():
        ...
        layout = CircuitRowLayout.build(
            graph, source_page_neurons=args.page_neurons, method=args.method
        )
    ...
    report = {
        ...
        "scheduled_locality": locality(
            graph, layout.row_order, layout.source_page_neurons
        ),
    }
    if args.build_only:
    ...
    candidate = ScheduledTritonCircuit(
        graph,
        args.batch_size,
        device=device,
        recurrent_matrix=recurrent,
    ...
    channels = np.ascontiguousarray(
        rng.random((base.input_count, args.batch_size), dtype=np.float32)
    )
    base.import_state(initial)
    candidate.import_state(initial)
    expected = base.step_numpy(channels, 0.05)
    ...
    for name, circuit in (("canonical", base), ("scheduled", candidate)):
        pure[name] = timed(
            lambda c=circuit: launch_recurrent_pair(c, drive, 0.05),
            device,
            args.warmup,
            args.iterations,
            args.repeats,
