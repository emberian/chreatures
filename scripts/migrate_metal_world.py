...
def migrate(source, output, snapshots, target_kernel, physics_backend=None):
    ...
    if output.exists():
        raise ValueError("Output must be a new manifest; original lives are never overwritten")
    ...
    if hashlib.sha256(native).hexdigest() != receipt["sha256"] or native[:8] != b"MBST1\0\0\0":
    ...
    if (metadata.get("version") not in (2, 3) or metadata.get("kernel") not in ("row", "simd")
    ...
    if len(payload) != 165122 * 4 * 4 * 3 or not np.isfinite(np.frombuffer(payload, dtype="<f4")).all():
        raise ValueError("Native rate/adaptation/support payload has invalid dimensions or values")
    ...
    migration = {
        "kind": "execution-backend-migration-v1", "world_id": state["id"], "tick": state["tick"],
        "source_world_sha256": source_hash, "source_neural_sha256": receipt["sha256"],
        "native_state_sha256": hashlib.sha256(payload).hexdigest(),
        "from_kernel": metadata["kernel"], "to_kernel": target_kernel,
    ...
    changed["neural_snapshot"] = {**receipt, "name": target_name,
                                   "sha256": hashlib.sha256(migrated_native).hexdigest(),
                                   "bytes": len(migrated_native)}
    if physics_backend is not None:
        if physics_backend not in ("reference", "vectorized") or state.get("body_mode") != "articulated":
    ...
    changed["journal"].append({"id": f"{state['id']}:{state['tick']}:execution-migration",
                               "time": float(state["world"]["mj_state"][0]), "kind": "implementation",
    ...
    with output.open("xb") as stream:
        stream.write(canonical({"format": envelope["format"], "sha256": digest, "state": changed}))
        stream.flush()
        import os
        os.fsync(stream.fileno())
    return {"output": str(output), "sha256": digest, "migration": migration,
...
def main():
    ...
    print(json.dumps(migrate(args.source, args.output, args.snapshot_dir,
                             args.kernel, args.physics_backend), indent=2))
