...
def measure(operation, device: torch.device, *, warmup: int, iterations: int,
...
def launch_baseline(circuit: TritonFusedCircuit, drive: torch.Tensor, dt: float) -> None:
    ...
    _csr_rate_substep[grid](*head, circuit.rates, circuit.rate_buffer,
        circuit.adaptation, circuit.support, drive, *tail, FINAL=False, **options)
    _csr_rate_substep[grid](*head, circuit.rate_buffer, circuit.rates,
        circuit.adaptation, circuit.support, drive, *tail, FINAL=True, **options)
...
def main() -> None:
    ...
    common = dict(device=device, recurrent_matrix=recurrent,
    ...
    circuits.update({f"tiled_e{edge}_w{warps}": EdgeTiledTritonCircuit(
    ...
    pure, complete = {}, {}
    for name, circuit in circuits.items():
        pure_op = ((lambda c=circuit: c._launch_pair(drive, .05))
                   if isinstance(circuit, EdgeTiledTritonCircuit)
                   else lambda c=circuit: launch_baseline(c, drive, .05))
        pure[name] = measure(pure_op, device, warmup=args.warmup,
    ...
    for _ in range(args.repeats):
        for name in order:
            ...
            timing = measure(lambda c=circuit: c.step_device(dev_channels, .05, validate=False),
                             device, warmup=1, iterations=args.iterations, repeats=1)
            complete_samples[name].append(float(timing["median_ms"]))
    for name, samples in complete_samples.items():
        complete[name] = {"median_ms": statistics.median(samples),
    ...
    report = {"graph": graph.summary(), "batch_size": args.batch_size,
              "candidate_metadata": {name: circuit.metadata() for name, circuit in circuits.items()},
              "parity": parity, "two_recurrent_update_kernels": pure,
              "complete_path_interleaved": complete,
              "speedup": {name: {"pure": base_pure / float(pure[name]["median_ms"]),
                                  "complete": base_complete / float(complete[name]["median_ms"])}
                          for name in circuits if name != "canonical_e1_w1"}}
    if args.output:
